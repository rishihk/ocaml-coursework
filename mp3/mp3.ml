...
(*Problem 12 other ways*)
let rec split_sum2 l f = 
  let rec split_sum_sol l f a b = 
  match l with 
  |  [] -> (a,b) 
  |  (x::xs) -> if(f x) then split_sum_sol xs f (a+x) b else split_sum_sol xs f a (b+x)
  in split_sum_sol l f 0 0

let rec split_sum3 l f = 
  let rec split_sum_sol l f (a, b) = 
  match l with 
  |  [] -> (a,b) 
  |  (x::xs) -> if(f x) then split_sum_sol xs f ((a+x),b) else split_sum_sol xs f (a, (b+x))
...
(*Problem 13*)
let even_count_fr_base = 0;; (*represnets the initial value of the accumulator b*)
let even_count_fr_rec x rec_val = if x mod 2 = 0 then rec_val + 1 else rec_val;;
  
(*Problem 14*)
let pair_sums_map_arg p = match p with | (a,b) -> a+b;;

(*Problem 15*)
let remove_even_base = [];;
let remove_even_rec n r = if not (n mod 2 = 0) then n::r else r;;
...
let split_sum_step f = fun (tr, fa) -> fun x -> if f x then (tr+x, fa) else (tr, fa+x);;
